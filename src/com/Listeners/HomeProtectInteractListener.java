...
public class HomeProtectInteractListener
  implements Listener
{
  @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true)
  public void onInteract(PlayerInteractEvent event) {
    try {
      event.getClickedBlock().getType(); Material.valueOf(Variable.Soil);
    ...
    if (!Util.Check(event.getPlayer(), 
        event.getPlayer().getLocation().getWorld().getName().replaceAll(Variable.world_prefix, "")).booleanValue()) {
