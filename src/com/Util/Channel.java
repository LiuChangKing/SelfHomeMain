...
public class Channel
{
  ...
  public static void waitToLoad(final Player p, final String Server, final String home_name) throws IOException {
    if (Bukkit.getWorld(String.valueOf(Variable.world_prefix) + home_name) != null) {
      int b; int i; Chunk[] arrayOfChunk; for (i = (arrayOfChunk = Bukkit.getWorld(String.valueOf(Variable.world_prefix) + home_name).getLoadedChunks()).length, b = 0; b < i; ) { Chunk chunk = arrayOfChunk[b];
        chunk.unload(true); b++; }
      
      Bukkit.unloadWorld(String.valueOf(Variable.world_prefix) + home_name, true);
    } 
        ...
        public void run()
        {
          ...
          if (Variable.world_prefix.equalsIgnoreCase("")) {
            if (Bukkit.getVersion().toString().toUpperCase().contains("ARCLIGHT") || Bukkit.getVersion().toString().contains("1.20.1")) {
              f = new File(String.valueOf(Variable.single_server_gen) + Variable.world_prefix);
            } else {
              f = new File(
                  String.valueOf(Variable.single_server_gen) + "world" + Variable.file_loc_prefix);
            }
