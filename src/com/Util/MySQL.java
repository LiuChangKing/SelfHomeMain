...
public class MySQL
{
  ...
  public static void autoUpdateServer() { (new BukkitRunnable()
      {
        public void run()
        {
          ...
          try {
            ps = con.prepareStatement("Select * From SelfHomeMain_Servers Where Server = ?",ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
  ...
  public static void addFlowersColumn() {
    ...
    try { ps = con.prepareStatement("alter table SelfHomeMain_Users add column flowers VARCHAR(100) default '0'");
      ps.executeUpdate(); }
    ...
    { 
      ...
      { e.printStackTrace(); }  } finally { try { if (res != null) res.close();  if (ps != null) ps.close();  if (con != null) con.close();  } catch (SQLException e) { e.printStackTrace(); }
       }
  ...
  public static void addAdvertisementColumn() {
    ...
    try { ps = con.prepareStatement("alter table SelfHomeMain_Users add column advertisement VARCHAR(255)");
      ps.executeUpdate(); }
  ...
  public static void addLimitBlockColumn() {
	    ...
	    try { ps = con.prepareStatement("alter table SelfHomeMain_Users add column limitblock VARCHAR(255)");
	      ps.executeUpdate(); }
  ...
  public static void addIconColumn() {
    ...
    try { ps = con.prepareStatement("alter table SelfHomeMain_Users add column icon VARCHAR(255)");
      ps.executeUpdate(); }
  ...
  public static void addVisitColumn() {
    ...
    try { ps = con.prepareStatement("alter table SelfHomeMain_Users add column visittime VARCHAR(255)");
      ps.executeUpdate(); }
  ...
  public static void addPopularityColumn() {
    ...
    try { ps = con.prepareStatement("alter table SelfHomeMain_Users add column popularity VARCHAR(100) default '0'");
      ps.executeUpdate(); }
  ...
  public static void addGiftColumn() {
    ...
    try { ps = con.prepareStatement("alter table SelfHomeMain_Users add column gifts TEXT default NULL");
      ps.executeUpdate(); }
  ...
  public static String getlocktime(String name) {
    ...
    try { ps = con.prepareStatement("SELECT locktime FROM SelfHomeMain_Users WHERE Name = ?");
      ps.setString(1, name);
      res = ps.executeQuery();
      if (res == null) {
        return result;
    ...
    { 
      ...
      { e.printStackTrace(); }  }  try { if (res != null) res.close();  if (ps != null) ps.close();  if (con != null) con.close();  } catch (SQLException e) { e.printStackTrace(); }
  ...
  public static int getAllWorldsAmount() {
    ...
    try {
      ps = con.prepareStatement("SELECT * FROM SelfHomeMain_Users",ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
      res = ps.executeQuery();
      res.last();
      amount = res.getRow();
    } catch (SQLException e) {
  ...
  public static void data_import(CommandSender p) {
    ...
    File folder = new File(
        String.valueOf(Main.JavaPlugin.getDataFolder().getPath().toString()) + Variable.file_loc_prefix + "playerdata");
  ...
  public static void data_export(CommandSender p) {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        File playerdata = new File(String.valueOf(Main.JavaPlugin.getDataFolder().getPath().toString()) + 
            Variable.file_loc_prefix + "playerdata" + Variable.file_loc_prefix + name + ".yml");
        if (playerdata.exists()) {
          String temp = Variable.Lang_YML.getString("ExportButHasAlreadyExist");
          if (temp.contains("<Name>")) {
            temp = temp.replace("<Name>", name);
          }
          p.sendMessage(temp);
        ...
        String temp = Variable.Lang_YML.getString("ExportSuccess");
        if (temp.contains("<Name>")) {
          temp = temp.replace("<Name>", name);
        }
        p.sendMessage(temp);
